...
text-align: center;
  ...
  <body>
    <table class="table table-striped table-bordered" style="width:30%;" align="center">
      ...
      <tbody>
        <?php
        $idpen = $_POST['penyanyi'];
        $n2 = 1;

        $qq2 = "SELECT music.title AS judul, music.durasi AS durasi, genre.name AS genre, singers.name AS penyanyi, music.photo AS photo, music.deskripsi AS deskripsi FROM music INNER JOIN genre ON music.id_genre=genre.id INNER JOIN singers ON music.id_singer=singers.id WHERE music.id_singer ='$idpen'";
